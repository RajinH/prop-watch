...
export function addPropertyToPortfolio(property: Property): Portfolio {
  ...
  const portfolio: Portfolio = existing ?? {
    properties: [],
...
export function loadHtagEstimates(
  addressKey: string,
): HtagEstimatesCache | null {
  if (!isClient()) return null;
  try {
...
export function saveHtagEstimates(
  addressKey: string,
  entry: HtagEstimatesCache,
): void {
  if (!isClient()) return;
  try {
    localStorage.setItem(
      HTAG_ESTIMATES_PREFIX + addressKey,
