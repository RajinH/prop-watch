      ...
      <div className="rounded-xl border border-green-200 bg-green-50 p-4 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1">
            ...
            >
              {TAX_BRACKETS.map((b) => (
                <option key={b.value} value={b.value.toString()}>
                  {b.label}
                </option>
              ))}
            </select>
    ...
    <div className="rounded-xl border border-green-200 bg-green-50 p-4 flex flex-col gap-3">
      <div className="flex items-center justify-between">
          ...
          <p className="text-sm font-semibold text-slate-800">
            ...
            {onTrack ? (
              ...
              <span className="text-slate-500">
                {fmt(goalProgress.monthly_gap)}/mo to go
              </span>
            )}
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => {
              setTargetInput(goalProgress.target.toString());
              setBracketInput(taxBracket.toString());
              setEditing(true);
            }}
      ...
      <p className="text-xs text-green-700">
        Target: {fmt(goalProgress.target)}/mo ·{" "}
        {fmt(Math.abs(goalProgress.monthly_gap))}/mo{" "}
        {onTrack ? "surplus" : "remaining"}
      </p>
    </div>
  );
}
